<?php
if (!defined('GWF_CHMOD'))
{
	define('GWF_CHMOD', 0777);
}
...
final class GWF_Log
{
	...
	public static function logException(Exception $e)
	{
		...
		self::log('critical', $message, self::GWF_CRITICAL);
		$log = GWF_Debug::backtraceException($e, true).PHP_EOL.self::stripPassword($_POST).PHP_EOL.$message;
...
//				$e->_throw( 'flushing logfile failed: content:' . $msg );
	...
	private static function writeLog($filename, $message)
	{
		...
		{
			return new GWF_Exception(sprintf('Cannot create logdir "%s" in %s line %s.', dirname($filename), __METHOD__, __LINE__));
		}
		...
		if (false === is_file($filename))
		{
			...
			if (false === $bool)
			{
				return new GWF_Exception(sprintf('Cannot create logfile "%s" in %s line %s.', $filename, __METHOD__, __LINE__));
			}
		}
		...
		{
			return new GWF_Exception(sprintf('Cannot write logs: logfile "%s" in %s line %s.', $filename, __METHOD__, __LINE__));
		}
